...
| 4   | P2       | **Global**       | `src/components/notifications/notification-bell.tsx` (new)    | RECEPTIONIST, PROVIDER | Bell icon in header; unread count badge (30s poll); dropdown with mark-as-read; click routes to calendar                                                                                                                                        | `getUnreadCount`, `getMyUnreadNotifications`, `markNotificationAsRead`, `markAllAsRead` |
...
| 15  | P2       | **Provider**     | `src/server/actions/availability.ts`                          | PROVIDER               | Auto-notify assigned receptionists when creating a leave request via `ProviderAssignment` query                                                                                                                                                 | `sendNotification` (inline via `createLeaveRequest`)                                    |
...
- [x] Auto-notify assigned receptionists on leave request (`createLeaveRequest` in `availability.ts`)
  - Queries `ProviderAssignment` for the provider, creates `Notification` for each with `leave_notification` type
  - Message: "Dr. [Name] requested a leave on [Date]. Please reschedule their appointments."
- [x] Server action: `sendNotification` (inline via `createLeaveRequest`)
...
- [x] Add a global notification bell icon in the header (`src/components/notifications/notification-bell.tsx`) visible to RECEPTIONIST and PROVIDER
  - Fetches `Notification` count where `receiverId === session.user.id` and `status === "UNREAD"` (polled every 30s)
  - Red badge with count (overflow: 99+)
  - Dropdown panel with recent unread notifications and mark-as-read action
  - Click notification marks as READ and routes to `/dashboard/calendar` for leave notifications
  - Server actions: `getUnreadCount`, `getMyUnreadNotifications`, `markNotificationAsRead`, `markAllAsRead`
...
| P2       | 3.3 Notify Receptionist      | 3.2            | 1 (action update)             |
| P2       | 5.3 Notification Badge       | 3.3            | 3 (component, hook, actions)  |
